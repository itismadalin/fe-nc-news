...
const request = axios.create({
  baseURL: "https://madalin-nc-news.herokuapp.com/api"
});
...
export const getTopics = async () => {
  ...
  const {
    data: { topics }
  } = await request.get(retrieveTopic);
  return topics;
};
...
export const fetchComments = async article_id => {
  ...
  const {
    data: { comments }
  } = await request.get(retrieveComments);
  return comments;
};
...
export const postComment = async ({ body, username, article_id }) => {
  ...
  const {
    data: { comment }
  } = await request.post(postedComments, { username, body });
  return comment;
};
